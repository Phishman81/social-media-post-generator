...
def add_bg_from_url():
    st.markdown(
        ...
        <style>
        .stApp {{
            background-image: url("https://cdn.midjourney.com/6a6876f7-0947-41b7-9e15-aeb0988ed0cc/0_0.webp");
    ...
    if st.button('Generate Post'):
        if openai_api_key and post_topic and post_goal and target_group:
            ...
            if example_text_for_style:
                ...
                prompt_text = f"Act as a highly experienced {platform} professional and craft a very compelling and interesting {platform} post out of the following information: the general topic of the post is: {post_topic}. Here are some specific details about the post content: {post_details}. The call-to-action should be: {call_to_action}. The target group is {target_group}, but do not specifically name it in your response. The goal of the post is {post_goal}. The selected language is {language}. Using the {hook_style}, the desired content length is {content_length}, with a preference for {hashtags} hashtags that suit the target group, {emojis} emojis, and a {list_type}. Please structure the post in the following way {post_structure}. In your response, capture the same tone and writing style and replicate the author's voice and how they express themselves in this text example: {example_text_for_style} - without incorporating any of the examples content."
            else:
                # Use the selected communication style otherwise
                prompt_text = f"Act as a highly experienced {platform} professional and craft a very compelling and interesting {platform} post out of the following information: the general topic of the post is: {post_topic}. Here are some specific details about the post content: {post_details}. The call-to-action should be: {call_to_action}. The target group is {target_group}, but do not specifically name it in your response. The goal of the post is {post_goal}. The communication style is {communication_style}. The selected language is {language}. Using the {hook_style}, the desired content length is {content_length}, with a preference for {hashtags} hashtags that suit the target group, {emojis} emojis, and a {list_type}. Please structure the post in the following way {post_structure}."
